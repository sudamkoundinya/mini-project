...
<div class="wrapper col3">
 ...
 <h2>Search Patient Details</h2>
                                                <form action="ShareActions.jsp" method="post">
                                                <table>
                                                    <tr><th>Patient Id</th><td><input type="text" name="pid" value="<%=pid%>" readonly="" style="height:30px;width:220px;"></td></tr>
                                                    <tr><th>Select group members to share</th><td>
                                                            <select name="gmid" required="">
                                                                <option></option>
                                                                <%try{
                                                                  ResultSet r=Queries.getExecuteQuery("select * from client where id!='"+id+"' and status='Joined'");
                                                                  while(r.next()){
                                                                      
                                                                  
                                                                %>
                                                                <option value="<%=r.getString("id")%>"><%=r.getString("uname")%></option>
                                                                <%}
}catch(Exception e){
out.println(e);
}%>
